...
V7.1.1 修掉一個真實使用者回報的死路：只建了基本資料、還沒建療程的病人，在資料庫頁選日期區間後會整個消失（連搜尋都找不到），而追蹤清單全以療程為基礎也不會出現 —— 等於這個病人被困死、永遠無法補建療程（坑 #13）。同時修掉同症狀的另一個潛在原因：`initDefaultSettings` 對舊資料庫早退，導致後加的設定（如「治療目的」）永遠空白而擋住新增療程（坑 #14）。前一版 V7.1.0 完成 PWA 升級。下版重點仍是把共享資料夾同步和 PWA 離線拉到醫院實機驗證。
