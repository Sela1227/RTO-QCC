...
class WeightFormView:
                        ...
                        ft.PopupMenuButton(
                            ...
                            items=[
                                ft.PopupMenuItem(
                                    text="手動 SDM",
                                    icon=ft.Icons.CHAT,
                                    on_click=lambda e: self._create_manual_intervention("sdm"),
                                ),
                                ft.PopupMenuItem(
                                    text="手動營養師轉介",
                                    icon=ft.Icons.RESTAURANT,
                                    on_click=lambda e: self._create_manual_intervention("nutrition"),
                                ),
                            ],
                        ),
                    ]),
    ...
    def _create_manual_intervention(self, intervention_type: str):
        """建立手動介入"""
        type_label = "SDM" if intervention_type == "sdm" else "營養師轉介"
        
        def on_confirm(e):
            self.intervention_service.create_manual(self.treatment_id, intervention_type)
            dialog.open = False
            self.main_view.show_snack(f"已建立{type_label}")
        ...
        dialog = ft.AlertDialog(
            title=ft.Text(f"手動{type_label}", font_family=SELATheme.FONT_FAMILY),
            content=ft.Text(f"確定要手動建立{type_label}記錄嗎？"),
            actions=[
