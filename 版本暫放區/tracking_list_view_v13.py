    ...
    def _build_card(self, treatment) -> ft.Control:
        ...
        if treatment.pending_interventions:
            badge = ft.Container(
                ...
                content=ft.Text("待處理", size=9, color=ft.Colors.WHITE),
            )
        ...
        elif treatment.last_weight and treatment.last_weight.alert_level == "sdm":
            badge = ft.Container(
        ...
        elif treatment.days_since_last and treatment.days_since_last <= 3:
            days_text = f"{treatment.days_since_last}天"
            days_color = SELATheme.SUCCESS
        elif treatment.days_since_last and treatment.days_since_last <= 7:
            days_text = f"{treatment.days_since_last}天"
            days_color = SELATheme.WARNING
        elif treatment.days_since_last:
            days_text = f"{treatment.days_since_last}天"
            days_color = SELATheme.DANGER
        else:
            days_text = ""
            days_color = SELATheme.TEXT_HINT
        ...
        if treatment.last_weight and treatment.last_weight.change_rate:
            rate = treatment.last_weight.change_rate
            rate_text = f"{rate:+.1f}%"
            if rate <= -5:
        ...
        return ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Row([
                    ft.Text(
                        treatment.patient.medical_id_display[-4:] if treatment.patient else "",
                ...
                ft.Row([
                    ...
                    ft.Container(expand=True),
                    ft.Text(rate_text, size=11, color=rate_color),
                ], spacing=2),
            ], spacing=2),
        )
