...
class MainView:
    ...
    def show_patient_form(self, prefill_medical_id: str = None):
        ...
        def on_save(e):
            ...
            if not all([mid, name, gender, birth_val, intent, cancer, baseline]):
            ...
            # 檢查病歷號
            existing = patient_service.find_by_medical_id(mid)
            if existing:
            ...
            patient = Patient(medical_id=mid, name=name, gender=gender, birth_date=birth_date)
            patient = patient_service.create(patient)
            ...
            treatment = treatment_service.create(treatment)
