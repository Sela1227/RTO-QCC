...
if __name__ == "__main__":
    ...
    # 檢查 flet 版本
    flet_version = getattr(ft, '__version__', '0.0.0')
    major_version = int(flet_version.split('.')[0]) if flet_version.split('.')[0].isdigit() else 0
    minor_version = int(flet_version.split('.')[1]) if len(flet_version.split('.')) > 1 and flet_version.split('.')[1].isdigit() else 0
    
    try:
        if major_version > 0 or minor_version >= 70:
            # flet 0.70+ 使用 ft.app() 但不需要 view 參數
            ft.app(target=main)
        else:
            # 舊版 flet
