...
class SettingsView:
                        ...
                        self._build_section("ℹ️ 關於", [
                            ft.Container(
                                ...
                                content=ft.Column([
                                    ...
                                    ft.Text("版本 2.0.0", size=12, color=SELATheme.TEXT_SECONDARY),
    ...
    def _show_threshold_rules_dialog(self, e):
        ...
        # 編輯/新增表單區（初始隱藏）
        editing_rule_id = [None]  # None = 新增, int = 編輯
        form_visible = [False]
        
        name_field = ft.TextField(label="名稱", width=120, text_size=12)
        ...
        threshold_field = ft.TextField(label="閾值 %", width=70, text_size=12, keyboard_type=ft.KeyboardType.NUMBER)
        ...
        form_title = ft.Text("新增規則", size=12, weight=ft.FontWeight.BOLD, color=SELATheme.PRIMARY)
        form_container = ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Row([
                    ft.TextButton("取消", on_click=lambda e: hide_form()),
                    ft.ElevatedButton("儲存", on_click=lambda e: save_rule(), bgcolor=SELATheme.PRIMARY, color=ft.Colors.WHITE),
                ]),
            ], spacing=6),
        )
        
        def show_edit_form(rule):
            """顯示編輯表單"""
            editing_rule_id[0] = rule["id"]
            form_title.value = "編輯規則"
            name_field.value = rule.get("name", "")
            type_dropdown.value = rule.get("type", "sdm")
            threshold_field.value = str(rule.get("threshold", 0))
            
            # 設置癌別勾選
            rule_cancers = rule.get("cancer_types", [])
            for cb in cancer_checkboxes:
                cb.value = cb.data in rule_cancers
        ...
        def show_add_form(e):
            """顯示新增表單"""
            editing_rule_id[0] = None
            form_title.value = "新增規則"
            name_field.value = ""
        ...
        def save_rule():
            ...
            if editing_rule_id[0]:
                # 編輯
                self.settings_service.update_threshold_rule(
                    editing_rule_id[0],
                    name=name,
                    type=rule_type,
                    threshold=threshold,
                    cancer_types=selected_cancers
                )
            else:
                # 新增
            ...
            for rule in rules:
                ...
                items.append(
                    ft.Container(
                        ...
                        border_radius=6,
                        ink=True,
                        on_click=lambda e, r=rule: show_edit_form(r),
                        content=ft.Row([
                            ft.Column([
        ...
        rule_list = ft.Column(build_rule_list(), spacing=4, scroll=ft.ScrollMode.AUTO, height=180)
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.Row([
                    ft.Text("現有規則（點擊可編輯）", size=12, color=SELATheme.TEXT_SECONDARY),
                    ft.Container(expand=True),
                    ft.TextButton("+ 新增", on_click=show_add_form),
                ]),
                rule_list,
                ft.Divider(),
                form_container,
            ], tight=True, width=420, scroll=ft.ScrollMode.AUTO),
            actions=[
                ft.ElevatedButton("完成", on_click=on_close),
            ],
        )
    ...
    def _show_staff_dialog(self, e):
                        ...
                        content=ft.Row([
                            ...
                            ft.Column([
                                ft.IconButton(
                                    ...
                                    icon_size=18,
                                    icon_color=SELATheme.TEXT_HINT if i == 0 else SELATheme.PRIMARY,
                                    on_click=lambda e, idx=i: move_up(idx),
                                    disabled=i == 0,
                                    padding=0,
                                ),
                                ft.IconButton(
                                    ...
                                    icon_size=18,
                                    icon_color=SELATheme.TEXT_HINT if i == len(staff_list)-1 else SELATheme.PRIMARY,
                                    on_click=lambda e, idx=i: move_down(idx),
                                    disabled=i == len(staff_list)-1,
                                    padding=0,
                                ),
                            ...
                            ft.Container(
                                expand=True,
                                ink=True,
                                on_click=lambda e, idx=i: start_edit(idx),
                            ...
                            ft.IconButton(
                                ...
                                icon_size=18,
                                icon_color=SELATheme.DANGER,
                                on_click=lambda e, idx=i: remove_staff(idx),
                            ),
                        ]),
                    )
                )
        ...
        def start_edit(idx):
            editing_index[0] = idx
            name_field.value = staff_list[idx]
            name_field.label = "編輯姓名"
            action_btn.text = "儲存"
            action_btn.bgcolor = SELATheme.PRIMARY
        ...
        def cancel_edit():
            ...
            name_field.label = "姓名"
            action_btn.text = "新增"
            action_btn.bgcolor = SELATheme.SUCCESS
        ...
        def save_staff():
            name = name_field.value.strip()
            if not name:
                self.main_view.show_snack("請輸入姓名")
                return
            
            if editing_index[0] is not None:
                # 編輯模式
                old_name = staff_list[editing_index[0]]
                if name != old_name and name in staff_list:
                    self.main_view.show_snack("人員已存在")
                    return
                staff_list[editing_index[0]] = name
        ...
        def remove_staff(idx):
            staff_list.pop(idx)
            self.settings_service.set_staff_list(staff_list)
            if editing_index[0] == idx:
                cancel_edit()
            build_staff_list()
            self.page.update()
        ...
        def move_up(idx):
            if idx > 0:
                staff_list[idx], staff_list[idx-1] = staff_list[idx-1], staff_list[idx]
                self.settings_service.set_staff_list(staff_list)
                build_staff_list()
                self.page.update()
        
        def move_down(idx):
            if idx < len(staff_list) - 1:
                staff_list[idx], staff_list[idx+1] = staff_list[idx+1], staff_list[idx]
                self.settings_service.set_staff_list(staff_list)
                build_staff_list()
                self.page.update()
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.Text("轉介人員列表（可拖曳排序）", size=13, color=SELATheme.TEXT_SECONDARY),
                ft.Container(height=8),
                ft.Container(
                    height=220,
