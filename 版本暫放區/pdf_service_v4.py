...
class PDFService:
    ...
    def generate_nutrition_referral(self, patient, treatment, referrer: str = "", 
        ...
        # 轉介需求選項（根據 needs 參數顯示勾選）
        opt_x = margin_left + 25*mm
        c.setFont(self.font_name, 10)
        
        # 勾選符號
        chk_subsidy = "☑" if needs.get("subsidy") else "□"
        chk_nutrition = "☑" if needs.get("nutrition") else "□"
        chk_wound = "☑" if needs.get("wound") else "□"
        chk_psychology = "☑" if needs.get("psychology") else "□"
        chk_rental = "☑" if needs.get("rental") else "□"
        chk_headwear = "☑" if needs.get("headwear") else "□"
        chk_support = "☑" if needs.get("support_group") else "□"
        chk_other = "☑" if needs.get("other") else "□"
        other_text = needs.get("other_text", "")
        
        c.drawString(opt_x, y - 8*mm, f"{chk_subsidy} 輔助諮詢（經濟、營養品、居家照顧、義乳胸衣）")
        c.drawString(opt_x, y - 18*mm, f"{chk_nutrition} 營養諮詢   {chk_wound} 傷口照護諮詢   {chk_psychology} 心理支持")
        c.drawString(opt_x, y - 28*mm, f"{chk_rental} 租借（假髮、輔具）  {chk_headwear} 頭巾/髮帽  {chk_support} 病友團體")
        other_display = f"：{other_text}" if other_text else ""
        c.drawString(opt_x, y - 38*mm, f"{chk_other} 其他{other_display}")
