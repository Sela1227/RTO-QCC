...
class ReportService:
    ...
    def get_intervention_effectiveness(self, start_date: date, end_date: date, filters: dict = None) -> dict:
        ...
        where_clauses = ["date(i.executed_at) BETWEEN ? AND ?", "i.status = 'completed'"]
        params = [start_str, end_str]
        ...
        interventions = self.db.fetch_all(
            f"""SELECT i.id, i.type, i.treatment_id, i.executed_at,
                       t.baseline_weight
               FROM interventions i
               JOIN treatments t ON i.treatment_id = t.id
               WHERE {where_sql}
               ORDER BY i.executed_at""", tuple(params)
        )
        ...
        for inv in interventions:
            ...
            if not executed_at or not baseline:
                continue
            ...
            weight_at_intervention = self.db.fetch_one(
                ...
                (treatment_id, executed_at.split('T')[0] if 'T' in str(executed_at) else executed_at)
            )
