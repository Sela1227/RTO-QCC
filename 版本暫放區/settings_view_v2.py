...
class SettingsView:
    ...
    def _show_threshold_dialog(self, type: str):
        """顯示閾值設定對話框"""
        titles = {
            "sdm": "SDM 提醒閾值",
            "nutrition": "營養師轉介閾值",
        ...
        field = ft.TextField(
            value=current_values.get(type, ""),
            keyboard_type=ft.KeyboardType.NUMBER,
            suffix_text="%" if type != "overdue" else "天",
            width=150,
        )
        
        def on_save(e):
            # TODO: 儲存設定
            self.main_view.show_snack("設定功能開發中")
            dialog.open = False
            self.page.update()
        ...
        dialog = ft.AlertDialog(
            title=ft.Text(titles.get(type, ""), font_family=SELATheme.FONT_FAMILY),
            content=ft.Column([
                ft.Text("此功能將在未來版本開放", size=12, color=SELATheme.TEXT_SECONDARY),
                ft.Container(height=8),
                field,
            ], tight=True),
            actions=[
