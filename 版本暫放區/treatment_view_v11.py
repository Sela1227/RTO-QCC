...
class TreatmentView:
                ...
                ft.Row([
                    ft.OutlinedButton(
                        ...
                        style=ft.ButtonStyle(
                            color=SELATheme.TEXT_SECONDARY,
                            side=ft.BorderSide(1, SELATheme.TEXT_HINT),
                        ),
                    ...
                    ft.OutlinedButton(
                        ...
                        style=ft.ButtonStyle(
                            ...
                            side=ft.BorderSide(1, SELATheme.SUCCESS),
                        ),
    ...
    def _show_complete_intervention_dialog(self):
        ...
        has_sdm = any(i.type == "sdm" for i in interventions)
    ...
    def _show_complete_with_confirm(self, e):
        """顯示結束療程對話框（需輸入病歷號確認）"""
        medical_id = self.treatment.patient.medical_id if self.treatment.patient else ""
        
        # 結束日期（預設今天）
        end_date = date.today()
        
        end_date_text = ft.Text(
            f"結束日期：{end_date.strftime('%Y-%m-%d')}",
            size=14,
        )
        
        confirm_field = ft.TextField(
            label=f"請輸入病歷號 {medical_id} 確認",
            hint_text="輸入病歷號",
            width=280,
        )
            ...
            def on_date_change(e):
                ...
                if picker.value:
                    ...
                    if hasattr(picked, 'date'):
                        end_date = picked.date()
                    else:
                        end_date = picked
                    end_date_text.value = f"結束日期：{end_date.strftime('%Y-%m-%d')}"
                    self.page.update()
        ...
        def on_confirm(e):
            # 驗證病歷號
            input_id = confirm_field.value.strip()
            if input_id != medical_id:
                error_text.value = "病歷號不符，請重新輸入"
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.Text(
                    f"確定要結束「{self.treatment.treatment_name}」療程嗎？",
                    size=14,
                ),
                ft.Container(height=12),
                ft.Row([
                    end_date_text,
                    ft.TextButton("選擇日期", on_click=show_end_date_picker),
                ]),
                ft.Container(height=12),
                ft.Container(
                    bgcolor="#FFF3E0",
                    ...
                    padding=SELATheme.SPACE_MD,
                    content=ft.Column([
                        ft.Text("⚠️ 結案後將不再追蹤此療程", size=12, weight=ft.FontWeight.BOLD),
                        ft.Text("請輸入病歷號碼確認", size=12, color=SELATheme.TEXT_SECONDARY),
                    ], spacing=4),
                ),
                ft.Container(height=12),
                confirm_field,
