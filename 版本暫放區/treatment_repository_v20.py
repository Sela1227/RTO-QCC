...
class TreatmentRepository:
    ...
    def get_ongoing_by_patient(self, patient_id: int) -> Optional[Treatment]:
        """取得未結束的療程（active 或 paused）"""
        row = self.db.fetch_one(
            "SELECT * FROM treatments WHERE patient_id = ? AND status IN ('active', 'paused')",
            (patient_id,)
        )
        return Treatment(**row) if row else None
    ...
    def update_baseline(self, id: int, weight: float, measure_date: date):
        """更新基準體重"""
        self.db.execute(
            """UPDATE treatments SET baseline_weight=?, baseline_date=?, 
               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
            (weight, measure_date.isoformat() if measure_date else None, id)
        )
