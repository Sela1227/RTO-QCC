...
class ReportView:
    ...
    def build(self) -> ft.Control:
        ...
        try:
            ...
            int_effect = self.report_service.get_intervention_effectiveness(self.start_date, self.end_date, self.filters)
            ...
            content=ft.Column([
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        ft.Container(height=16),
                        # 介入成效（體重回升率）
                        self._build_intervention_effectiveness(int_effect),
                        ft.Container(height=16),
                        # 追蹤品質
    ...
    def _build_intervention_effectiveness(self, effect: dict) -> ft.Control:
        """建立介入成效統計（體重回升率）"""
        overall = effect.get("overall", {})
        sdm = effect.get("sdm", {})
        nutrition = effect.get("nutrition", {})
        ...
        return ft.Container(
            ...
            padding=SELATheme.SPACE_MD,
            content=ft.Column([
                ft.Text("介入成效（體重回升率）", size=14, weight=ft.FontWeight.BOLD),
                ft.Text("介入後體重回升超過 0.5kg 或變化率改善超過 1%", size=10, color=SELATheme.TEXT_HINT),
                ft.Container(height=12),
                # 總體
                ft.Row([
                    ft.Column([
                        ft.Text(f"{overall.get('rate', 0):.0f}%", size=28, weight=ft.FontWeight.BOLD, 
                               color=SELATheme.SUCCESS if overall.get('rate', 0) >= 50 else SELATheme.WARNING),
                        ft.Text("整體回升率", size=11, color=SELATheme.TEXT_SECONDARY),
                        ft.Text(f"({overall.get('improved', 0)}/{overall.get('total', 0)})", size=10, color=SELATheme.TEXT_HINT),
                    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                    ft.Container(width=30),
                    ft.Column([
                        ft.Text(f"{sdm.get('rate', 0):.0f}%", size=20, weight=ft.FontWeight.BOLD, color=SELATheme.WARNING),
                        ft.Text("SDM 後回升", size=10, color=SELATheme.TEXT_SECONDARY),
                        ft.Text(f"({sdm.get('improved', 0)}/{sdm.get('total', 0)})", size=9, color=SELATheme.TEXT_HINT),
                    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                    ft.Container(width=20),
                    ft.Column([
                        ft.Text(f"{nutrition.get('rate', 0):.0f}%", size=20, weight=ft.FontWeight.BOLD, color=SELATheme.DANGER),
                        ft.Text("營養師後回升", size=10, color=SELATheme.TEXT_SECONDARY),
                        ft.Text(f"({nutrition.get('improved', 0)}/{nutrition.get('total', 0)})", size=9, color=SELATheme.TEXT_HINT),
                    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
