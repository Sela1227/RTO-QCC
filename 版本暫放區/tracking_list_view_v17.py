...
class TrackingListView:
    ...
    def build(self) -> ft.Control:
        ...
        if treatments:
            ...
            for i in range(0, len(cards), 4):
                ...
                card_rows.append(ft.Row(controls=row_cards, spacing=8, alignment=ft.MainAxisAlignment.START))
            
            card_area = ft.Container(
                expand=55,
                alignment=ft.alignment.top_left,  # 從頂端開始
                content=ft.Column(card_rows, spacing=8, scroll=ft.ScrollMode.AUTO, 
    ...
    def _build_card(self, treatment) -> ft.Control:
        ...
        # 邊框：選中=藍色粗框，否則只有左邊框
        if is_selected:
            border = ft.border.all(3, "#1976D2")  # 藍色外框
        else:
    ...
    def _update_detail(self, treatment):
        ...
        info_section = ft.Column([
            ft.Row([
                ft.Text(f"{patient.medical_id_display}" if patient else "", size=20, weight=ft.FontWeight.BOLD),
                ft.Text(f"{patient.name}" if patient else "", size=20, weight=ft.FontWeight.BOLD),
                ft.Text(f"（{patient.age}歲）" if patient and patient.age else "", size=16, color=SELATheme.TEXT_SECONDARY),
            ], spacing=8),
            ft.Row([
                ft.Text(f"{treatment.cancer_type_label}", size=14, color=SELATheme.TEXT_SECONDARY),
                ft.Text(f"基準 {treatment.baseline_weight:.1f} kg", size=14, color=SELATheme.TEXT_SECONDARY),
                ft.Container(expand=True),
                ft.Container(
                    bgcolor={"active": SELATheme.SUCCESS, "paused": SELATheme.WARNING}.get(treatment.status, SELATheme.TEXT_HINT),
                    border_radius=4,
                    padding=ft.padding.symmetric(horizontal=8, vertical=2),
                    content=ft.Text(treatment.status_label, size=11, color=ft.Colors.WHITE),
                ),
            ], spacing=8),
        ], spacing=4)
        ...
        if treatment.pending_interventions:
            ...
            if has_sdm:
                btns.append(ft.ElevatedButton("執行 SDM", on_click=lambda e, t=treatment: self._go_intervention(t, "sdm"),
                                              bgcolor=SELATheme.WARNING, color=ft.Colors.WHITE, height=38))
            if has_nutrition:
                btns.append(ft.ElevatedButton("轉營養師", on_click=lambda e, t=treatment: self._go_intervention(t, "nutrition"),
                                              bgcolor=SELATheme.DANGER, color=ft.Colors.WHITE, height=38))
            ...
            intervention_section = ft.Container(
                ...
                padding=SELATheme.SPACE_SM,
                content=ft.Column([
                    ft.Text("⚠️ 待處理介入", size=13, weight=ft.FontWeight.BOLD, color=SELATheme.WARNING),
        ...
        if len(records) >= 2:
            ...
            chart_section = ft.Container(
                height=130,
                ink=True,
                on_click=lambda e, t=treatment, r=records: self._show_chart_fullscreen(t, r),
                content=ft.Stack([
                    chart_card,
                    ft.Container(
                        alignment=ft.alignment.top_right,
                        padding=4,
                        content=ft.Icon(ft.Icons.FULLSCREEN, size=18, color=SELATheme.TEXT_HINT),
                    ),
                ]),
            )
        ...
        for r in records[:6]:
            ...
            weight_items.append(
                ft.Container(
                    ...
                    content=ft.Row([
                        ft.Text(date_str, size=13, width=45),
                        ft.Text(f"{r.weight:.1f} kg", size=13, weight=ft.FontWeight.BOLD),
                        ft.Text(r.change_rate_display, size=12, color=SELATheme.TEXT_SECONDARY),
                        ft.Container(expand=True),
        ...
        records_section = ft.Column([
            ft.Text("體重記錄", size=14, weight=ft.FontWeight.BOLD),
        ...
        if treatment.status == "active":
            action_buttons = ft.Row([
                ft.OutlinedButton("手動介入", height=36, on_click=lambda e, t=treatment: self._show_manual_intervention_dialog(t)),
                ft.OutlinedButton("無法測量", height=36, on_click=lambda e, t=treatment: self._show_unable_dialog(t)),
                ft.Container(expand=True),
                ft.OutlinedButton("暫停", height=36, style=ft.ButtonStyle(color=SELATheme.WARNING),
                                 on_click=lambda e, t=treatment: self._show_pause_dialog(t)),
                ft.OutlinedButton("結案", height=36, style=ft.ButtonStyle(color=SELATheme.SUCCESS),
                                 on_click=lambda e, t=treatment: self._show_complete_dialog(t)),
            ], spacing=8)
        elif treatment.status == "paused":
            action_buttons = ft.Row([
                ft.ElevatedButton("恢復療程", height=38, bgcolor=SELATheme.SUCCESS, color=ft.Colors.WHITE,
                                 on_click=lambda e, t=treatment: self._on_resume(t)),
                ft.OutlinedButton("終止", height=36, style=ft.ButtonStyle(color=SELATheme.DANGER),
    ...
    def _show_chart_fullscreen(self, treatment, records):
        """顯示全螢幕體重曲線圖"""
        chart_component = WeightChartComponent(treatment.baseline_weight, records, width=600, height=400)
        chart_card = chart_component.build()
        ...
        dialog = ft.AlertDialog(
            title=ft.Row([
                ft.Text("體重趨勢圖", font_family=SELATheme.FONT_FAMILY),
                ft.Container(expand=True),
