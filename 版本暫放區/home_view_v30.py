...
class HomeView:
    ...
    def build(self) -> ft.Control:
        ...
        # 統計卡片（加寬）- 點擊跳到對應篩選
        cards = ft.Row([
            self._build_stat_card("🔴", "一週未量", summary["overdue_count"], SELATheme.DANGER, "overdue"),
            self._build_stat_card("🟡", "待量測", summary["pending_count"], SELATheme.WARNING, "pending"),
        ], alignment=ft.MainAxisAlignment.CENTER, spacing=20)
        
        cards2 = ft.Row([
            self._build_stat_card("🟠", "待 SDM", summary["sdm_pending"], SELATheme.WARNING, "sdm"),
            self._build_stat_card("🔴", "待轉營養", summary["nutrition_pending"], SELATheme.DANGER, "nutrition"),
        ], alignment=ft.MainAxisAlignment.CENTER, spacing=20)
    ...
    def _build_stat_card(self, icon: str, label: str, count: int, color: str, filter_type: str = None) -> ft.Control:
        """建立統計卡片（放大）"""
        # 0 顯示綠燈
        display_icon = "🟢" if count == 0 else icon
        display_color = SELATheme.SUCCESS if count == 0 else color
        ...
        return ft.Container(
            ...
            content=ft.Column([
                ft.Text(display_icon, size=28),
                ft.Text(
                    str(count),
                    size=40,
                    weight=ft.FontWeight.BOLD,
                    color=display_color if count > 0 else SELATheme.SUCCESS,
                ),
                ft.Text(
                    label,
                    size=16,
