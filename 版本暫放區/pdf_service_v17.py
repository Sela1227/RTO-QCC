...
class PDFService:
    ...
    def generate_nutrition_referral(self, patient, treatment, referrer: str = "", 
                                    phone: str = "", notes: str = "", needs: dict = None) -> str:
        """
        ...
        def draw_checkbox(x, y, checked):
            """繪製勾選框"""
            box_size = 3*mm
            c.rect(x, y - 2.5*mm, box_size, box_size)
            if checked:
                # 畫打勾符號
                c.setLineWidth(0.8)
                c.line(x + 0.5*mm, y - 0.5*mm, x + 1.2*mm, y - 2*mm)
                c.line(x + 1.2*mm, y - 2*mm, x + 2.8*mm, y + 0.8*mm)
                c.setLineWidth(0.5)
            return x + box_size + 1.5*mm
        
        # 第一行：輔助諮詢
        row_y = y - 8*mm
        text_x = draw_checkbox(opt_x, row_y, needs.get("subsidy", False))
        c.drawString(text_x, row_y, "輔助諮詢（經濟、營養品、居家照顧、義乳胸衣）")
        
        # 第二行：營養諮詢、傷口照護、心理支持
        row_y = y - 18*mm
        text_x = draw_checkbox(opt_x, row_y, needs.get("nutrition", False))
        c.drawString(text_x, row_y, "營養諮詢")
        text_x = draw_checkbox(opt_x + 35*mm, row_y, needs.get("wound", False))
        c.drawString(text_x, row_y, "傷口照護諮詢")
        text_x = draw_checkbox(opt_x + 75*mm, row_y, needs.get("psychology", False))
        c.drawString(text_x, row_y, "心理支持")
        
        # 第三行：租借、頭巾、病友團體
        row_y = y - 28*mm
        text_x = draw_checkbox(opt_x, row_y, needs.get("rental", False))
        c.drawString(text_x, row_y, "租借（假髮、輔具）")
        text_x = draw_checkbox(opt_x + 50*mm, row_y, needs.get("headwear", False))
        c.drawString(text_x, row_y, "頭巾/髮帽")
        text_x = draw_checkbox(opt_x + 85*mm, row_y, needs.get("support_group", False))
        c.drawString(text_x, row_y, "病友團體")
        
        # 第四行：其他
        row_y = y - 38*mm
        text_x = draw_checkbox(opt_x, row_y, needs.get("other", False))
        other_text = needs.get("other_text", "")
        other_display = f"其他：{other_text}" if other_text else "其他"
        c.drawString(text_x, row_y, other_display)
