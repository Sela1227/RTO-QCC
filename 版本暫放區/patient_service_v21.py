...
class PatientService:
    ...
    def get_with_treatments(self, patient_id: int) -> Patient:
        ...
        if patient:
            ...
            # active_treatment: 只有 active 狀態
            patient.active_treatment = next(
                (t for t in patient.treatments if t.status == "active"), None
            )
            # ongoing_treatment: active 或 paused 狀態（未結束的療程）
            patient.ongoing_treatment = next(
                (t for t in patient.treatments if t.status in ("active", "paused")), None
            )
        return patient
