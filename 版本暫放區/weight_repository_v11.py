...
class WeightRepository:
    ...
    def get_by_id(self, record_id: int) -> Optional[WeightRecord]:
    ...
    def update(self, record: WeightRecord) -> WeightRecord:
        """更新體重記錄"""
        measure_date = record.measure_date
        if isinstance(measure_date, date):
            measure_date = measure_date.isoformat()
        ...
        self.db.execute(
            """UPDATE weight_records 
               SET measure_date = ?, weight = ?, change_rate = ?, alert_level = ?, notes = ?
               WHERE id = ?""",
            (measure_date, record.weight, record.change_rate, 
             record.alert_level, record.notes, record.id)
        )
        return record
    ...
    def delete(self, record_id: int) -> bool:
        ...
        # 先刪除相關的介入記錄
        self.db.execute(
            "DELETE FROM interventions WHERE weight_record_id = ?",
            (record_id,)
        )
        # 刪除體重記錄
        self.db.execute(
