...
class MainViewV2:
    ...
    def __init__(self, page: ft.Page, auth_service: AuthService, on_logout):
        self.page = page
    ...
    def _build_ui(self):
        ...
        user_bar = ft.Container(
            ...
            content=ft.Row([
                ...
                ft.Text(
                    f"👤 {user.display_name}（{user.role_label}）",
                    size=12,
                ...
                ft.TextButton(
                    "用戶管理",
                    on_click=lambda e: self._show_user_management(),
                ) if user.is_admin else ft.Container(),
                ft.TextButton(
                    "登出",
    ...
    def _show_user_management(self):
        ...
        self.page.controls.clear()
        user_view = UserManagementView(self.page, self.auth_service, self._back_to_main)
        self.page.add(user_view.build())
        self.page.update()
    ...
    def _confirm_logout(self):
        ...
        dialog = ft.AlertDialog(
            title=ft.Text("確認登出", font_family=SELATheme.FONT_FAMILY),
            content=ft.Text("確定要登出嗎？"),
            actions=[
                ft.TextButton("取消", on_click=cancel),
                ft.ElevatedButton("登出", on_click=do_logout, bgcolor=SELATheme.DANGER, color=ft.Colors.WHITE),
            ],
        )
