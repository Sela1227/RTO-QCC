...
class TrackingListView:
    ...
    def _build_card(self, treatment) -> ft.Control:
        ...
        return ft.Container(
            ...
            content=ft.Column([
                # 行1：病歷號 姓名
                ft.Row([
                    ft.Text(medical_id, size=12, weight=ft.FontWeight.BOLD, color=SELATheme.TEXT_PRIMARY),
                    ft.Text(name, size=12, max_lines=1, overflow=ft.TextOverflow.ELLIPSIS),
                ], spacing=6),
                # 行2：癌別 年紀
                ft.Row([
    ...
    def _update_detail(self, treatment):
        ...
        if treatment.unable_to_measure:
            unable_hint = ft.Container(
                ...
                content=ft.Row([
                    ft.Icon(ft.Icons.INFO_OUTLINE, size=18, color=SELATheme.PRIMARY),
                    ft.Text(f"目前狀態：無法測量（{treatment.unable_reason_label}）", 
    ...
    def _quick_add_weight(self, treatment):
        ...
        # 先驗證輸入
        try:
            weight_str = self.weight_input.value.strip() if self.weight_input.value else ""
            if not weight_str:
                self.main_view.show_snack("請輸入體重")
                return
            ...
            self.main_view.show_snack("請輸入正確的體重數字")
            return
        ...
        try:
            ...
            # 顯示實際錯誤訊息（如：該日期已有記錄）
            self.main_view.show_snack(str(e))
