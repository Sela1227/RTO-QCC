...
# 設置 matplotlib 後端（必須在 import pyplot 之前）
# 如果沒安裝 matplotlib 也能運行
...
def get_chinese_fonts():
    ...
    if system == "Windows":
        return ['Microsoft JhengHei', 'SimHei', 'Microsoft YaHei']
    elif system == "Darwin":  # macOS
        return ['PingFang TC', 'Heiti TC', 'STHeiti', 'Arial Unicode MS']
    else:  # Linux
        return ['WenQuanYi Micro Hei', 'Noto Sans CJK TC', 'DejaVu Sans']
...
class WeightChartComponent:
    ...
    def build(self, width: int = 380, height: int = 220) -> ft.Control:
        ...
        if not MATPLOTLIB_AVAILABLE:
            return self._build_placeholder("需安裝 matplotlib")
    ...
    def _build_placeholder(self, message: str = None) -> ft.Control:
        """資料不足時的佔位"""
        msg = message or "資料不足，需要至少 2 筆記錄"
        return ft.Container(
            bgcolor=SELATheme.SURFACE,
            border_radius=SELATheme.RADIUS_MD,
    ...
    def _generate_chart(self, width: int, height: int) -> str:
        ...
        try:
            # 設定中文字體（跨平台）
            plt.rcParams['font.sans-serif'] = get_chinese_fonts()
